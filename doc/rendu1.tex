...
SBRS & $if(R_r(b) = 1) PC \leftarrow PC + 2 \text{ or } 3$ & Skip if bit in register set & 1/2/3 &  & None & \\
SBIC & $if(I/O(A, b) = 0) PC \leftarrow PC + 2 \text{ or } 3$ & Skip if bit in $I/O$ register cleared & 1/2/3 &  & None & \\
SBIS & $if(I/O(A, b) = 1) PC \leftarrow PC +2 \text{ or } 3$ & Skip if bit in $I/O$ register set & 1/2/3 &  & None & \\
BRBS & $if(SREG(s) = 1) PC \leftarrow PC + k + 1$ & Branch if status flag set & 1/2 &  & None & \\
BRBC & $if(SREG(s) = 0) PC \leftarrow PC + k + 1$ & Branch if status flag cleared & 1/2 &  & None & \\
BREQ & $if(Z = 1) PC \leftarrow PC + k + 1$ & Branch if equal & 1/2 &  & None & \\
BRNE & $if(Z = 0) PC \leftarrow PC + k + 1$ & Branch if not equal & 1/2 &  & None & \\
BRCS & $if(C = 1) PC \leftarrow PC + k + 1$ & Branch if carry set & 1/2 &  & None & \\
BRCC & $if(C = 0) PC \leftarrow PC + k + 1$ & Branch if carry cleared & 1/2 &  & None & \\
BRSH & $if(C = 0) PC \leftarrow PC + k + 1$ & Branch if same or higher & 1/2 &  & None & \\
BRLO & $if(C = 1) PC \leftarrow PC + k + 1$ & Branch if lower & 1/2 &  & None & \\
BRMI & $if(N = 1) PC \leftarrow PC + k + 1$ & Branch if minus & 1/2 &  & None & \\
BRPL & $if(N = 0) PC \leftarrow PC + k + 1$ & Branch if plus & 1/2 &  & None & \\
BRGE & $if(N \text{ xor } V = 0) PC \leftarrow PC + k + 1$ & Branch if greater or equal, signed & 1/2 &  & None & \\
BRLT & $if(N \text{ xor } V = 1) PC \leftarrow PC + k + 1$ & Branch if less than zero, signed & 1/2 &  & None & \\
BRHS & $if(H = 1) PC \leftarrow PC + k + 1$ & Branch if half carry flag set & 1/2 &  & None & \\
BRHC & $if(H = 0) PC \leftarrow PC + k + 1$ & Branch if half carry flag cleared & 1/2 &  & None & \\
BRTS & $if(T = 1) PC \leftarrow PC + k + 1$ & Branch if $T$ flag set & 1/2 &  & None & \\
BRTC & $if(T = 0) PC \leftarrow PC + k + 1$ & Branch if $T$ flag cleared & 1/2 &  & None & \\
